...
CacheSetDBPV_DYN::CacheSetDBPV_DYN(
   ...
   , m_set_info(set_info)
{
   m_rrip_bits = new UInt8[m_associativity];    // m_rrip_bits is an array of length associativity
   for (UInt32 i = 0; i < m_associativity; i++)
      m_rrip_bits[i] = m_rrip_insert;

    /* To record how many times a block got hit */
    m_block_access = new UInt8[m_associativity];  // m_block_access is an array of length associativity
    ...
    if (0 == g_iteration_count)
    {
        ...
        * reference count */
       for (UInt32 g_phaseID = 0; g_phaseID < NUM_PHASES; g_phaseID++) // NUM_PHASES =100
       {
                                  ...
                                  UInt8 m_rrip_max, UInt8 m_rrip_insert, UInt8 coreID)
{
    ...
    if (0 == coreID)
    {
        if (g_InsValidBlocksC0 == m_saturation_counter_max_value) //If the number of inserted blocks due to c0 reaches the max value of counter then new phase
        {
            db_percent_c0 = (10000 * g_ValidDeadBlocksC0 / g_InsValidBlocksC0);
            //db_percent_c0 = (10000 * g_ValidDeadBlocksC0 / sampler_access_c0); 	
        ...
        else
        {
             ...
             g_core0_insert = m_rrip_insert;  // insertion at LRU -1
        }
    }
    ...
    if (1 == coreID)
    {
        if (g_InsValidBlocksC1 == m_saturation_counter_max_value)
        {
            ...
            //db_percent_c1 = (10000 * g_ValidDeadBlocksC1 / sampler_access_c1);
...
UInt32
CacheSetDBPV_DYN::InsertBlockAtIndex(UInt32 index, core_id_t core_id) // returns the index i.e. the block to be evicted
{
    /* When we found a victim block, we are finding how many blocks have
     * same RRPV value
     */
	 
	/* We are evicting a block. Hence we have to check if the block is a 
	deadblock or not. We check its number of accesses (m_block_access ). If
	it is zero we say it is dead. Then we reset the number of accesses to 0.
	[ARINDAM]*/
	...
	/////////////////[Arindam]///////////////////////////
	/* I think this portion needs to be edited. We need to check if a block 
		is dead or not from the predictor 
	*/
	/* I create 2 arrays for sampler set. Both have a length of 32*12 since in sampler
		there are 32 sets and 12 ways. sampler_tag keeps the tags and sampler_deadblockpredictor
		keeps the predicted bit.

	   I also create a predictor_confidence array of 3bits. Lets assume threshold is 6. 
	   	It has 4096 entries. I also create a predictor_signature array of 15bits. It also has 4096
	   	entries.

	   Only thing I need is the tag of the block of LLC accessed and PC of the instruction accessing it.

	   Another thing I need to confirm with the authors is index_predictor. I think it is 12bits and hence 
	   it can access 4096 entries of predictor table. which means the hash function will convert 15bits 
	   signature (truncated PC) to 12 bits index_predictor. If we can use 3 different hash functions, accuracy
	...
	for (i=0; i<(32*12); i++)
	{
		if (sampler_tag(i)==tag)				//i determines sampler index
		{
			if (core_id==0)
				sampler_access_c0++;
			else if (core_id==1)
				sampler_access_c1++;
			index_predictor=hash.signature; 	//signature = truncated PC
			for (j=0; j<4096; j++) 				//j determines index of predictor
			{
				if(index_predictor==predictor_signature(j))
				{

					if (flag_miss==1)			//in case of LLC miss, predictor_confidence(j)++;
						{
						predictor_confidence(j)++;
						flag_miss==0;
						}						
					else if (flag_hit==1)		//in case of LLC miss, predictor_confidence(j)++;
						{
						predictor_confidence(j)--;
						flag_hit==0;
						}						
					...
					if (predictor_confidence(j)<=6)
						sampler_deadblockpredictor(i)=0;		//0 means living block
					else
						sampler_deadblockpredictor(i)=1;
				}
			}	
		}
	}
	*/
    ...
    /*
	for (index=0; index<(32*12); index++)
	{
		if (tag==sampler_tag(index))
		{
			if(sampler_deadblockpredictor(index)==1)
	...
	*/
    ...
    if (0 == m_block_access[index]) // if block is accessed 0 times	
    {
        /* Block is dead, findout who was its owner */
        (0 == m_block_owner[index]) ? g_numTotalDeadBlocksC0++ : g_numTotalDeadBlocksC1++;
        (0 == m_block_owner[index]) ? g_ValidDeadBlocksC0++  : ,++;
    ...
    {
        m_rrip_bits[index] = g_core1_insert;  // What if m_rrip_bits[index] = m_rrip_insert
    ...
    if (g_InsValidBlocksC0 == m_saturation_counter_max_value)   // number of invalid blocks reaches max value of counter
    ...
    if (m_saturation_counter_max_value == g_InsValidBlocksC1)
    {
        ...
        g_InsValidBlocksC1  = 0; // shouldn't this be halved??
        ///////[ARINDAM]///////////
        /*
        sampler_access_c1 = 0;
        */
...
CacheSetDBPV_DYN::getReplacementIndex(CacheCntlr *cntlr, core_id_t core_id)
{
   ...
   for(UInt32 j = 0; j <= m_rrip_max; ++j)
   {
      ...
      for (UInt32 i = 0; i < m_associativity; i++)
      {
         if (m_rrip_bits[i] < m_rrip_max)	// If there is no block with RRPV of m_rrip_max, increment RRPV value of all blocks
         {
            m_rrip_bits[i]++;
         }
      }
